...
//	@Success		200			{object}	ds.PageDataResponse{list=dto.FilesystemItem}
...
func (c *FilesystemController) MoveFiles(ctx *fiber.Ctx, claim *jwt.Claims, id uint64, body *ds.IDsForm) error {
	return c.filesystemSvc.MoveFiles(ctx.Context(), claim.TenantID, claim.UserID, id, body.IDs)
}
...
func (c *FilesystemController) CopyFiles(ctx *fiber.Ctx, claim *jwt.Claims, id uint64, body *ds.IDsForm) error {
	return c.filesystemSvc.CopyFiles(ctx.Context(), claim.TenantID, claim.UserID, id, body.IDs)
}
