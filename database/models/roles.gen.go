// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.

package models

import (
	"time"

	"gorm.io/gorm/schema"
)

const TableNameRole = "roles"

// Role mapped from table <roles>
type Role struct {
	ID          uint64    `gorm:"column:id;type:bigint(20) unsigned;primaryKey;autoIncrement:true;comment:ID" json:"id"` // ID
	CreatedAt   time.Time `gorm:"column:created_at;type:datetime(3);comment:创建时间" json:"created_at"`                     // 创建时间
	Name        string    `gorm:"column:name;type:varchar(128);not null;comment:名称" json:"name"`                         // 名称
	Slug        string    `gorm:"column:slug;type:varchar(128);not null;comment:唯一标识" json:"slug"`                       // 唯一标识
	Description string    `gorm:"column:description;type:varchar(256);comment:描述" json:"description"`                    // 描述
	ParentID    uint64    `gorm:"column:parent_id;type:bigint(20) unsigned;comment:父角色" json:"parent_id"`                // 父角色
}

func (*Role) TableName(namer schema.Namer) string {
	if namer == nil {
		return TableNameRole
	}
	return namer.TableName(TableNameRole)
}
