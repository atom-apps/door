...
func routeUserController(engine fiber.Router, controller *controller.UserController) {
	...
	engine.Get(strings.TrimPrefix("/v1/users/:id<int>", basePath), DataFunc1(controller.Show, Integer[uint64]("id", PathParamError)))
	...
	engine.Put(strings.TrimPrefix("/v1/users/:id<int>", basePath), Func2(controller.Update, Integer[uint64]("id", PathParamError), Body[dto.UserForm](BodyParamError)))
	engine.Delete(strings.TrimPrefix("/v1/users/:id<int>", basePath), Func1(controller.Delete, Integer[uint64]("id", PathParamError)))
}
