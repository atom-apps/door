...
type Filesystem struct {
	...
	DriverID  uint64                   `gorm:"column:driver_id;type:bigint(20) unsigned;comment:驱动" json:"driver_id"`                  // 驱动
	...
	Type      uint32                   `gorm:"column:type;type:tinyint(3) unsigned;not null;comment:类型" json:"type"`                   // 类型
	ParentID  uint64                   `gorm:"column:parent_id;type:bigint(20) unsigned;comment:父级ID" json:"parent_id"`                // 父级ID
	Status    uint32                   `gorm:"column:status;type:tinyint(3) unsigned;comment:状态" json:"status"`                        // 状态
	Mime      string                   `gorm:"column:mime;type:varchar(256);comment:MIME" json:"mime"`                                 // MIME
	Ext       string                   `gorm:"column:ext;type:varchar(32);comment:后缀名" json:"ext"`                                     // 后缀名
	Size      uint64                   `gorm:"column:size;type:bigint(20) unsigned;comment:文件大小" json:"size"`                          // 文件大小
	...
	Metadata  model.FilesystemMetadata `gorm:"column:metadata;type:longblob;comment:元数据" json:"metadata"`                              // 元数据
}
